...
Modified for Half-Run (Top 81 trials, half wordlist).
"""
...
detailed_intro_text = ('IN THE FOLLOWING TASK, YOU WILL BE ASKED TO RATE ONE OF THREE TYPES OF JUDGMENTS FOR DIFFERENT WORDS:\n\n'
                       ...
                       'FOR EACH ITEM, PLEASE RESPOND WITH YES [right pointer finger] OR \nNO [right middle finger].\n'
...
#%%
expName = 'self_other_half'
input_subID = 0
# expInfo box
expInfo = {'setting': ['SCANNER', 'PRACTICE'],
           'words_file': ['wordlist_run1.csv', 'wordlist_run2.csv', 'wordlist_runBackup.csv', 'wordlist_runprac.csv'],
           'subID': str(input_subID),
           'sessionID': ['Baseline', 'Repeat_Baseline', 'T6', 'T12'],
           'runID': ['1', '2', 'prac']}
dlg = gui.DlgFromDict(dictionary=expInfo, title='My Experiment (HALF RUN)')
if dlg.OK == False:
    core.quit()
...
sequence_file = 'sequence_run' + expInfo['runID'] + '_half.csv'
runID = expInfo['runID']
...
word_lists = word_lists.groupby(['condition_design', 'valence']).head(9).reset_index(drop=True)
# ----------------------------------------------------------------------
...
if setting == 'PRACTICE':
    ...
    # --- MODIFICATION 3: Halved target time to account for half-run length ---
    target_time = 330  
    # -------------------------------------------------------------------------
